...
Run the one-command Mailpit smoke test before manually configuring n8n Send an Email credentials:

```powershell
python scripts/run_v2_mailpit_email_capture_test.py
```

Manual n8n SMTP credential setup is optional after this script passes. The script verifies dry-run generation, HTML export, SMTP capture, and Mailpit inbox inspection without n8n credentials.
