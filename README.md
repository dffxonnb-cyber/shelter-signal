...
V2 기능은 아직 실제 알림 서비스로 구현된 상태가 아닙니다. 현재 브랜치에서는 `mart.alert_candidates` SQL model과 email digest preview export 기반을 준비하고 있으며, 실제 이메일 발송은 구현하지 않았습니다. n8n workflow documentation과 알림 자동화는 다음 단계로 분리합니다. 앱 동작이나 DB schema는 V1 상태를 유지합니다. 자세한 범위는 [docs/v2-roadmap.md](docs/v2-roadmap.md)와 [docs/email-digest-preview.md](docs/email-digest-preview.md)를 참고합니다.
