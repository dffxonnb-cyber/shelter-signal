"""Smoke test for the public animal shelter information API.
...
JSON_OUTPUT_PATH = RAW_DIR / "shelter_api_sample.json"
XML_OUTPUT_PATH = RAW_DIR / "shelter_api_sample.xml"
...
def request_once(url: str) -> tuple[int, str, str]:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "shelter-signal-shelter-api-smoke-test/0.1",
        },
    )
...
def print_json_summary(status_code: int, data: Any, saved_path: Path) -> None:
    ...
    print(f"Sample shelter item keys: {format_keys(sample_item_keys)}")
...
def print_xml_or_text_summary(status_code: int, body: str, saved_path: Path) -> None:
    try:
        ...
        print("Sample shelter item keys: (none)")
        print(f"Saved raw sample: {saved_path.relative_to(PROJECT_ROOT)}")
        return
...
def main() -> int:
    ...
    try:
        ...
        print(f"Shelter API smoke test could not run: {exc}", file=sys.stderr)
        return 1
